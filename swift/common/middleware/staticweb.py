...
By deafult, the listings will be rendered with a label of
...
class _StaticWebContext(WSGIContext):
    ...
    def _get_container_info(self, env):
        ...
        if is_success(container_info['status']):
            ...
            # TODO(hurricanerix): This should probably be fixed somewhere
            #                     else...
            if str(type(self._listings_label)) == "<type 'unicode'>":
                self._listings_label = self._listings_label.encode('utf-8')
    ...
    def _listing(self, env, start_response, prefix=None):
        ...
        """
        label = cgi.escape(env['PATH_INFO'])
        if self._listings_label:
            groups = cgi.escape(env['PATH_INFO']).split('/')
            label = '{0}/{1}'.format(
                self._listings_label, '/'.join(groups[4:]))
