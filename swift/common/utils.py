...
def get_hmac(request_method, path, expires, key):
    ...
    :param expires: Unix timestamp as an int for when the URL
                    expires.
    :param key: Key str, secret key to be used with the HMAC-SHA1.
...
def get_swift_info(admin=False, disallowed_sections=[]):
    """
    ...
    for section in _swift_info:
        ...
        if section not in info:
            info[section] = {}
        for key, value in _swift_info[section].iteritems():
...
def register_swift_info(name='swift', admin=False, **kwargs):
    ...
    """
    if name == 'admin':
        raise KeyError('\'admin\' is reserved name.')
